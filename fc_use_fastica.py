...
def fetch_dataset(n):
    dataset = fetch_lfw_people(min_faces_per_person=n)   # labelled faces in the wild data with users more than 100 faces
...
def dimensionality_reduction_ICA(n_components, X_train, height, width):
    ...
    t0 = time()
    ica = FastICA(n_components=n_components, whiten=True, max_iter = 500).fit(X_train)
...
plot_images(X[:6], y[:6], height, width, n_row=2, n_col=3)
